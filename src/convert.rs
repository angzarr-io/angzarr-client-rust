...
pub fn unpack<T: prost::Message + Default + Name>(any: &Any) -> Result<T> {
    ...
    T::decode(any.value.as_slice()).map_err(|e| ClientError::InvalidArgument(format!("decode error: {}", e)))
}
...
pub fn proto_to_uuid(proto: &ProtoUuid) -> Result<Uuid> {
    Uuid::from_slice(&proto.value).map_err(|e| ClientError::InvalidArgument(format!("invalid UUID: {}", e)))
}
...
pub fn parse_timestamp(rfc3339: &str) -> Result<Timestamp> {
    let dt: DateTime<Utc> = rfc3339.parse().map_err(|e| ClientError::InvalidTimestamp(format!("{}: {}", rfc3339, e)))?;
